...
@exceptions: This function can throw exceptions
*/
bool QRCodeStateEstimator::estimateStateFromGrayscaleImageMsg(const sensor_msgs::ImageConstPtr msg, cv::Mat &inputCameraPoseBuffer, std::string &inputQRCodeIdentifierBuffer, double &inputQRCodeDimensionBuffer)
{
    std::vector<cv::Mat> cameraPosesBuffer;
    std::vector<std::string> QRCodeIdentifiersBuffer;
    std::vector<double> QRCodeDimensionBuffer;
    ...
    returnValue = estimateOneOrMoreStatesFromGrayscaleImageMsg(msg, cameraPosesBuffer, QRCodeIdentifiersBuffer, QRCodeDimensionBuffer);
...
@exceptions: This function can throw exceptions
*/
bool QRCodeStateEstimator::estimateOneOrMoreStatesFromGrayscaleImageMsg(const sensor_msgs::ImageConstPtr msg, std::vector<cv::Mat> &inputCameraPosesBuffer, std::vector<std::string> &inputQRCodeIdentifiersBuffer, std::vector<double> &inputQRCodeDimensionsBuffer){
...
//Clear the buffers to store everything in
    ...
    for (zbar::Image::SymbolIterator symbol = zbarFrame.symbol_begin();  symbol != zbarFrame.symbol_end();  ++symbol)
    {
        ...
        double QRCodeDimensionInMeters;
        std::string QRCodeIdentifierString;
        if(!extractQRCodeDimensionFromString(symbol->get_data(), QRCodeDimensionInMeters, QRCodeIdentifierString))
        {
            continue; //Couldn't read dimension
        }
...
//Make sure it updates every frame, even if it found the qr code in the last frame
    ...
    return inputCameraPosesBuffer.size() > 0;
