...
public class DecentralizedAction extends Action{
    ...
    public ActionStatus enterAction(Time time){
        ...
        Ts_Matrix = new SimpleMatrix(Ni+1, Ni+1);
    ...
    public ActionStatus loopAction(Time time) {
        if(time.subtract(timeStamp).compareTo(timeOut) > 0){
            ...
            for(int k = 1; k < Hp + 1;++k) {
                X.insertIntoThis(k * 2 * (Ni + 1), 0, A.mult(X.extractMatrix((k - 1) * 2 * (Ni + 1), k * 2 * (Ni + 1) + 1, 0, SimpleMatrix.END)));
            }
            ...
            for(int l = Hu; l<Hp+1;++l){
                for(int k = 0; k <l-Hu+1;++k ){
                    PU.insertIntoThis((l-1)*2*(Ni +1),(Hu-1)*(Ni+1),array_pow(A,l-k).mult(B).plus(PU.extractMatrix((l-1)*2*(Ni+1),l*2*(Ni+1) + 1,(Hu-1)*(Ni+1),Hu*(Ni+1) + 1)));
                }
            }
            ...
            U = E.mult(PU).transpose().mult(E).mult(PU).plus(R).solve(E.mult(PU).transpose().mult(E.mult(PX).mult(xi).plus(S.negative())));
            ...
            des_velocity = u.extractVector(true,0).scale(Ts).plus(temp_matrix);
            double des_speed = des_velocity.normF();

            if(des_speed>Vmax){
                des_velocity.scale(Vmax/des_speed);
            }
